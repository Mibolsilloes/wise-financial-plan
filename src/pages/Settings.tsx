...
// ─── Plans tab (extracted to access usePlan hook safely) ───
function PlansTab() {
  const { profile, updateProfile } = useAuth();
  const currentPlan = (profile as { plan?: string } | null)?.plan === "premium" ? "premium" : "free";
  const { usage, limits } = usePlan();

  const handleSwitchPlan = async (target: "free" | "premium") => {
    const { error } = await updateProfile({ plan: target } as Partial<{ plan: "free" | "premium" }>);
    if (error) toast.error("No se pudo cambiar el plan");
    else toast.success(target === "premium" ? "¡Plan Premium activado!" : "Plan Gratuito activado");
  };

  return (
    <div className="space-y-6 max-w-4xl">
      {/* Current usage card (only for Free) */}
      {currentPlan === "free" && (
        <div className="glass rounded-xl p-6 border border-border/50">
          <h3 className="text-base font-semibold mb-4">Tu uso actual</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <UsageStat label="Transacciones este mes" used={usage.transactionsThisMonth} max={limits.transactionsPerMonth as number} />
            <UsageStat label="Cuentas bancarias" used={usage.bankAccounts} max={limits.bankAccounts as number} />
            <UsageStat label="Tarjetas de crédito" used={usage.creditCards} max={limits.creditCards as number} />
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className={cn(
          ...
          currentPlan === "free" ? "border-primary/50 shadow-glow-primary" : "border-border/50"
        )}>
        ...
        <div className={cn(
          ...
          currentPlan === "premium" ? "border-2 border-warning shadow-glow-primary" : "border-2 border-warning/50"
        )}>
      ...
      <p className="text-xs text-muted-foreground">
        * El cobro real se habilitará próximamente. Por ahora puedes activar el plan Premium para probar todas las funciones sin límites.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="p-4 rounded-lg bg-muted/30 border border-border">
      ...
      <p className={cn("text-xl font-bold", isWarn && "text-warning")}>
        {used}<span className="text-sm text-muted-foreground font-normal"> / {isInf ? "∞" : max}</span>
      </p>
      {!isInf && (
